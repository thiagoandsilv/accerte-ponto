...
Também dá pra informar a hora extra manualmente: ao editar um registro (aba Relatórios → Editar), tem dois
campos — "Início" e "Fim" da hora extra — igual aos horários de entrada/saída. Quando preenchidos, esse
intervalo **substitui** o cálculo automático (útil pra hora extra combinada à parte, que não é só "passou
de 8h") e, como toda edição manual, o registro cai na aprovação do gestor com o motivo "Hora extra
(manual)". Deixe os dois em branco para voltar a usar o cálculo automático; preencher só um dos dois dá
erro (tem que ser os dois ou nenhum).
...
Na aba Relatórios tem um seletor de mês e o botão "Espelho (PDF)", ao lado do "Exportar CSV". Ele gera um
PDF formatado (data, entrada, intervalo, saída, horas, extra e observação de cada dia, mais os totais do
mês e linhas de assinatura) pronto pra imprimir ou anexar. Quem não é gestor sempre recebe o próprio
espelho; gestor precisa selecionar uma pessoa específica no filtro (não funciona com "Toda a equipe").
...
- `PUT /api/records/:id` — edição manual `{ entrada, inicioIntervalo, fimIntervalo, saida, observacao, extraManualEntrada?, extraManualSaida? }`
  (requer sessão; só o dono do registro ou um gestor pode editar; marca o registro como `pendente`;
  `extraManualEntrada`/`extraManualSaida` sobrescrevem o cálculo automático de hora extra — envie os dois ou
  nenhum dos dois; a resposta inclui `extraManualMin`, a duração já calculada a partir desse intervalo)
- `GET /api/records?start=&end=&employeeId=` — relatório por período (requer sessão; `employeeId` só tem
  ...
  parâmetro)
- `GET /api/records/espelho?month=YYYY-MM&employeeId=` — gera o espelho de ponto do mês em PDF (requer
  sessão; `employeeId` só tem efeito para gestor — quem não é gestor sempre recebe o próprio espelho)
