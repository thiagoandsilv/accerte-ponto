...
-- Hora extra informada manualmente (em minutos), sobrescrevendo o cálculo automático (>8h).
ALTER TABLE records ADD COLUMN IF NOT EXISTS extra_manual_min INTEGER;

-- Hora extra manual como início/fim (em vez de só a duração), pra poder reabrir e ver os horários.
ALTER TABLE records ADD COLUMN IF NOT EXISTS extra_manual_entrada TIMESTAMPTZ;
ALTER TABLE records ADD COLUMN IF NOT EXISTS extra_manual_saida TIMESTAMPTZ;
