...
function recordRowToJson(row) {
  return {
    ...
    extraManualMin: row.extra_manual_min == null ? null : Number(row.extra_manual_min),
  };
}
...
app.put('/api/records/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    const { entrada, inicioIntervalo, fimIntervalo, saida, observacao, extraManualMin } = req.body || {};

    let extraManualVal = null;
    if (extraManualMin !== undefined && extraManualMin !== null && extraManualMin !== '') {
      const n = Number(extraManualMin);
      if (!Number.isFinite(n) || n < 0) return res.status(400).json({ error: 'Horas extras manuais inválidas.' });
      extraManualVal = Math.round(n);
    }
    ...
    const { rows } = await pool.query(
      `UPDATE records SET
         ...
         status = 'pendente', extra_manual_min = $6
       WHERE id = $7 RETURNING *`,
      [entrada || null, inicioIntervalo || null, fimIntervalo || null, saida || null, (observacao || '').trim(), extraManualVal, id]
    );
    if (rows.length === 0) return res.status(404).json({ error: 'Registro não encontrado.' });
    const withName = await pool.query('SELECT r.*, e.name AS employee_name FROM records r JOIN employees e ON e.id = r.employee_id WHERE r.id = $1', [id]);
    res.json(recordRowToJson(withName.rows[0]));
